...
interface VoteService {
    ...
    @GET("/api/v1/rooms/{roomId}/candidates")
    suspend fun getCandidates(
        @Path("roomId") roomId: Long,
        @Query("category") category: String
    ): ApiResponse<List<CandidateResponse>>

    @POST("/api/v1/candidates/{candidateId}/vote")
    suspend fun castVote(
        @Path("candidateId") candidateId: Long
    ): ApiResponse<Boolean>

    @DELETE("/api/v1/rooms/{roomId}/votes/reset")
    suspend fun resetVotes(
        @Path("roomId") roomId: Long,
        @Query("category") category: String
    ): ApiResponse<Boolean>

    @GET("/api/v1/rooms/{roomId}/candidates/results")
